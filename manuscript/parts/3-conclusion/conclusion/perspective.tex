...
We think that there is three possibles definition of an uncertain array, the last one being the combination of the first two:
\begin{itemize}
	\item an uncertain array is a collection when the collection itself is uncertain,
	\item an uncertain array is a collection that contains uncertain element,
	\item an uncertain array is a collection that contains uncertain element and when the collection itself is uncertain.
\end{itemize}
...
These operations have no effect on the uncertainty of uncertain arrays that respect the second definition, if we consider the operation as certain.
...
Let us detail the sorting of an array.
If the full collection is uncertain, common sorting algorithm can be applied without impacting the uncertainty.
In the other cases, algorithms should be redesigned.
Indeed, as uncertainty propagation is mapped to operators and each algorithm will use differently the comparison operators, the result can differ.
Sorting the first integer array ([54, 10, 3, 24]), a MergeSort makes 5 comparison while a QuickSort makes 4. 
These algorithms should consider uncertainty at a higher level than language operator.
...
For example, using a descriptive language that add meta-information to uncertain values.
...
This language can define some information that can be used to quantity or reduce the uncertainty.
For example, minimal and maximal values can be used to check if the receive value is in this range or not.
...
In the listing, \textit{att1} is approximatively equal to the double of \textit{att2}.
