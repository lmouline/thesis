...
if ! command -v texlua > /dev/null; then
  ...
  ./install-tl --profile=../manuscript/texlive/texlive.profile

  cd ..
fi

echo "Run: tlmgr update --self --all"
tlmgr update --self
tlmgr update --all

# Just including texlua so the cache check above works
echo "Run: tlmgr install luatex"
...
echo "Run: tlmgr install collection-langeuropean"
tlmgr install collection-langeuropean

# Common fonts with hard to debug errors if not found
echo "Run: tlmgr install collection-fontsrecommended"
...
echo "RUN: cd $(dirname ${BASH_SOURCE[0]})"
cd "$(dirname "${BASH_SOURCE[0]}")"
echo "Run: tlmgr install $(cat texlive_packages)"
tlmgr install $(cat texlive_packages)

# Keep no backups (not required, simply makes cache bigger)
echo "Run: tlmgr option -- autobackup 0"
