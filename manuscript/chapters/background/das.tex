...
\Glspl{adptSyst} can be adapted automatically in response to changes in their \gls{env} or of the systems themselves.
...
Furthermore, each \gls{adptSyst} is composed of four elements: the \gls{env}, the managed system, adaptation goals, and the managing systems~\cite{DBLP:books/sp/19/Weyns19}.
...
This modification will then create new data that correspond to an output context. In the remainder of this paper, we refer to the output context as impacted context, or impact(s).
