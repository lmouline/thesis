...
%We detail this example in Section~\ref{sec:lux-sg-scenario}.
...
%In this article, we propose \languageName{}, a language able to represent uncertain data as built-in language types along with their supported operations. 
%This language extends common primitive data types with uncertainty while redefining operational semantics of their supported operators to enable a transparent propagation of uncertainty.
%\languageName{} supports the following probability distribution: Gaussian, Bernoulli, binomial, Dirac delta function, and Rayleigh.
%These fours distributions are a sampling of distributions which cover the different data types: uncertain booleans, continuous or discrete uncertain numbers, and uncertain reference.

In this article, we describe the following contributions: 
\begin{itemize}
	\vspace{-0.5em}
